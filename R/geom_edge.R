...
#' @export
GeomEdgeLabel <- ggproto('GeomEdgeLabel', GeomPath,
  draw_panel = function(self, data, panel_scales, coord, arrow = NULL,
    ...
    # Calculate the path, but only return textGrob
    ...
    if (any(!is.na(data$label))) {
      ...
      gTree(children=gList(label_grob))
    } else {
...
GeomEdgeParallelPath <- ggproto('GeomEdgeParallelPath', GeomEdgePath,
  draw_panel = function(data, panel_scales, coord, arrow = NULL, sep = unit(2, 'mm'),
    ...
    if (inherits(panel, 'gTree')) {
      ...
      class(child[[1]]) <- c('parallelPath', class(child[[1]]))
      panel$children <- child
    } else if (inherits(panel, 'gList')) {
